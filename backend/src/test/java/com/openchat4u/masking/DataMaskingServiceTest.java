...
class DataMaskingServiceTest {
    ...
    void testFindByTenant() {
        ...
        when(maskingRuleRepository.selectList(any(Wrapper.class))).thenReturn(expected);

        List<DataMaskingRule> result = maskingService.findByTenant(tenantCode);

        assertEquals(2, result.size());
        verify(maskingRuleRepository).selectList(any(Wrapper.class));
    }
    ...
    void testCreate() {
        ...
        when(maskingRuleRepository.insert(any(DataMaskingRule.class))).thenAnswer(invocation -> {
    ...
    void testDelete() {
        ...
        when(maskingRuleRepository.selectById(1L)).thenReturn(existing);
        when(maskingRuleRepository.updateById(any(DataMaskingRule.class))).thenReturn(1);

        maskingService.delete(1L);

        assertFalse(existing.getIsActive());
        verify(maskingRuleRepository).updateById(existing);
    }

    @Test
    void testDeleteNotFound() {
        when(maskingRuleRepository.selectById(999L)).thenReturn(null);
    ...
    void testApplyMaskingNoRules() {
        ...
        when(maskingRuleRepository.selectList(any(Wrapper.class))).thenReturn(List.of());
    ...
    void testApplyMaskingFull() {
        ...
        when(maskingRuleRepository.selectList(any(Wrapper.class))).thenReturn(List.of(rule));
    ...
    void testApplyMaskingMultipleRules() {
        ...
        when(maskingRuleRepository.selectList(any(Wrapper.class))).thenReturn(Arrays.asList(phoneRule, emailRule));
